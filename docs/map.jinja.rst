...
We will see a quick setup to configure the ``docker`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``docker`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``docker`` formulas under ``/srv/salt/docker/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/docker/parameters/dns:domain/
    ln -s dns:domain /srv/salt/docker/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/docker/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/docker/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``docker`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``docker`` formulas under ``/srv/salt/docker/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/docker/parameters/roles
...
We create a configuration for the role ``docker/server`` in ``/srv/salt/docker/parameters/roles/docker/server.yaml``:
...
We create another configuration for the role ``docker/client`` in ``/srv/salt/docker/parameters/roles/docker/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles docker/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles docker/client
...
.. code-block:: console
    ...
    mv /srv/salt/docker/parameters/dns:domain/  '/srv/salt/docker/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/jinja/map.jinja" import mapdata as docker with context %}

    test-does-nothing-but-display-docker-as-json:
      test.nop:
        - name: {{ docker | json }}
...
Here is an example based on `template-formula/docker/config/file.sls`_:
...
.. code-block:: sls
    ...
    docker-config-file-file-managed:
      file.managed:
        - name: {{ docker.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='docker-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ docker.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``docker`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ docker['winner'] }}
...
.. _template-formula/docker/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/docker/config/file.sls
